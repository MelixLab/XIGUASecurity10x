...
impl TreeEnsemble {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<TreeEnsemble, String> {
        ...
        for t in 0..n_trees {
            ...
            for i in start..end {
                ...
                if feature_id < 0 {
                    // 叶子：紧凑格式要求恰好 1 个 (class, weight)（LightGBM 多分类结构保证）。
                    ...
                    // 子下标 → 树内相对下标；加载时校验后，评估路径无需再做边界检查。
    ...
    pub fn evaluate(&self, features: &[f32]) -> TreeOutput {
        ...
        for t in &self.trees {
            ...
            // 树内切片：所有相对子下标在加载时已校验 < size。
            let tree = &self.nodes[base..base + size];
            let mut idx = 0usize;
            // 单棵树最多遍历 size 步，防止异常数据成环导致死循环。
            ...
            loop {
                ...
                // BRANCH_LEQ: feature <= threshold -> true 分支，否则 false 分支。
                // NaN 时 `<=` 为 false → 走 false 分支（与原实现一致）。
