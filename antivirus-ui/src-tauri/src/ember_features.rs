...
/// Total feature dimensions expected by the model.
pub const NDIM: usize = 2568;

/// Extract a 2568-dim f32 feature vector from raw bytes.
...
const STRING_REGEX_PATTERNS: &[&str] = &[
    ...
    r"\b(?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2})\b",               // 32 email_addr
...
fn string_regex_set() -> &'static RegexSet {
    ...
    SET.get_or_init(|| {
        ...
        RegexSet::new(STRING_REGEX_PATTERNS).expect("STRING_REGEX_PATTERNS must compile")
    })
}
...
const MACHINE_TYPES: &[u16] = &[
    0x0000, 0x014c, 0x0162, 0x0166, 0x0168, 0x0169, 0x0184, 0x01a2,
    0x01a3, 0x01a4, 0x01a6, 0x01a8, 0x01c0, 0x01c2, 0x01c4, 0x01d3,
    0x01f0, 0x01f1, 0x0200, 0x0266, 0x0284, 0x0284, 0x0366, 0x0466,
    0x0520, 0xc0ee, 0x0EBC, 0x5032, 0x5064, 0x5128, 0x6232, 0x6264,
    0x8664, 0x9041, 0xAA64, 0xC0EE,
];

const SUBSYSTEM_TYPES: &[u16] = &[
    0, 1, 2, 3, 5, 7, 8, 9, 10, 11, 12, 13, 14, 16,
];
...
fn parse_rich_header_values(bytez: &[u8]) -> Vec<u32> {
    ...
    let dans_marker: [u8; 4] = [0x44, 0x61, 0x6E, 0x53];
...
fn authenticode_signature(bytez: &[u8]) -> Vec<f32> {
    ...
    out[0] = num_certs as f32;
    out[5] = num_certs as f32;
    out
}
